...
function install (gyp, argv, callback) {
  ...
  var iojsMode = (gyp.opts.iojs | false)
  var distUrl = gyp.opts['dist-url'] || gyp.opts.disturl || 'https://' + (iojsMode ? 'iojs.org' : 'nodejs.org') + '/dist'
  var distFilePrefix = (iojsMode ? 'iojs' : 'node')
  ...
  function go () {
    ...
    mkdir(devDir, function (err, created) {
      ...
      var tarballUrl = tarPath ? tarPath : distUrl + '/v' + version + '/' + distFilePrefix + '-v' + version + '.tar.gz'
        , badDownload = false
        , extractCount = 0
      ...
      function afterTarball () {
        ...
        if (win) {
          ...
          async++
          downloadNodeLib(deref, iojsMode)
        }
      ...
      function downloadNodeLib (done, iojsMode) {
        ...
        var dir32 = path.resolve(devDir, 'ia32')
          ...
          , checkSumKey32 = 'node.lib'
          , checkSumKey64 = 'x64/node.lib'

        if(iojsMode) {
          // if iojsMode, overwrite some variables
          nodeLibUrl32 = distUrl + '/v' + version + '/win-x86/iojs.lib'
          nodeLibUrl64 = distUrl + '/v' + version + '/win-x64/iojs.lib'
          checkSumKey32 = 'win-x86/iojs.lib'
          checkSumKey64 = 'win-x64/iojs.lib'
        }
        ...
        mkdir(dir32, function (err) {
          ...
          req.on('response', function (res) {
            ...
            getContentSha(res, function (_, checksum) {
              contentShasums[checkSumKey32] = checksum
              log.verbose('content checksum', checkSumKey32, checksum)
            })
        ...
        mkdir(dir64, function (err) {
          ...
          req.on('response', function (res) {
            ...
            getContentSha(res, function (_, checksum) {
              contentShasums[checkSumKey64] = checksum
              log.verbose('content checksum', checkSumKey64, checksum)
            })
